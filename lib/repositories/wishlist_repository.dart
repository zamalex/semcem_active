...
class WishListRepository {
  ...
  Future<dynamic> isProductInUserWishList({product_id = 0}) async {
    String url =
        ("${AppConfig.BASE_URL}/wishlists-check-product/${product_id}");
    final response = await ApiRequest.get(
        url: url,
        headers: {
          "Authorization": "Bearer ${access_token.$}",
  ...
  Future<dynamic> add({product_id = 0}) async {
    String url =
        ("${AppConfig.BASE_URL}/wishlists-add-product/${product_id}");
  ...
  Future<dynamic> remove({product_id = 0}) async {
    String url =
        ("${AppConfig.BASE_URL}/wishlists-remove-product/${product_id}");
    final response = await ApiRequest.get(
        url: url,
        headers: {
          "Authorization": "Bearer ${access_token.$}",
